...
This module contains production-grade tests for ai_cli.ai_chat that have been
updated to incorporate recent fixes and improvements:

- Deterministic fake ask() via autouse fixture:
        - An autouse fixture `mock_ask` monkeypatches the ask() symbol both in this
          test module's globals and in the original ai_cli.ai_chat module so tests
          never call external APIs.
        - The fake implementation validates arguments and returns either a mocked
          successful response or an error-prefixed string beginning with "[ERROR]".
        - Recognizes an explicit INVALID_PROVIDER sentinel used by tests to assert
          unknown-provider handling.
        - Handles model override semantics (accepts valid overrides, returns a safe
          error string for explicit invalid overrides like "invalid-model" or "").

- Pytest shim for import-time resilience:
        - A minimal shim is included to allow this module to be imported in
          environments without pytest installed (useful for linters / static tools).
        - The shim exposes no-op decorators/fixtures that emulate pytest.mark,
          pytest.fixture, and pytest.parametrize sufficiently for import and basic
          execution of docstring examples.

- Clear constants and helper utilities:
        - Canonical constants (VALID_PROVIDER, INVALID_PROVIDER, VALID_PROMPT,
          VALID_MODEL, EXPECTED_RESPONSE_KEYWORD) centralize test expectations.
        - Helper is_error_response() detects error-formatted strings produced by ask()
          to keep assertions readable and consistent.

- Test practices and annotations:
        - Integration tests are explicitly marked with @pytest.mark.integration and
          timeboxed with @pytest.mark.timeout to avoid CI hangs.
        - Tests use strong typing, descriptive docstrings, and defensive assertions
          to give actionable failure messages.
        - Tests cover normal flows, model overrides, invalid inputs, provider/registry
          structure, and basic latency smoke checks.

Note:
- The fake ask() is intentionally deterministic to exercise calling code paths
  and error handling without network calls. If you change error message
  prefixes or sentinel values, update the corresponding assertions/constants
  in these tests.
"""
