...
class RAGPipeline:
        ...
        - upsert_documents: store chunk+embeddings in an in-memory DB
        ...
        def _embed_one(self, text: str) -> List[float]:
                ...
                production use — swap in a model-based embedder for real apps.
                """
                vec: List[float] = []
        ...
        def upsert_documents(
                ...
                """
                Index/Upsert provided documents. Each document is chunked and
                embedded and stored in the in-memory vector DB.
                """
                if doc_ids is None:
                ...
                logger.info(
                        "Indexed %d documents -> %d chunks total",
                        len(doc_texts),
                        len(self._store),
                )
        ...
        while True:
                try:
                        ...
                        if inspect.isawaitable(result) or isinstance(result, AsyncIterable):
                                return asyncio.run(
                                        _drain_async_result(
                                                result, kwargs.get("provider", "unknown"),
