...
_STDIN_MAX_BYTES: int = (
    102_400  # 100 KiB — same ceiling as the prompt char limit
)
...
def _build_ask_kwargs(
    ...
    try:
        ...
        logger.debug(
            "Failed to inspect ask() signature; sending best-effort args"
        )
        base: dict[str, Any] = {
            "provider": provider,
            "prompt": prompt,
...
def _invoke_with_retries(
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            sleep_secs = min(
                backoff * (2 ** (attempt - 1)), _MAX_BACKOFF_SECONDS
            )
...
def run_interactive(
    ...
    while True:
                ...
                if os.path.exists(payload):
                    ...
                    if safe_path is None:
                        ...
                        try:
                            ...
                            print(
                                f"Failed to index file: {exc}", file=sys.stderr
                            )
                else:
                    pipeline.upsert_documents(
                        [payload],
        ...
        if exit_code not in (0, 130):
            print(
                f"[ERROR] command failed with code {exit_code}", file=sys.stderr
            )
...
def _load_rag_docs(
    ...
    for payload in rag_docs:
        if os.path.exists(payload):
            ...
            if safe_path is None:
                logger.warning(
                    "Skipping %r: path contains illegal traversal sequences.",
                    payload,
                )
                continue
            try:
    ...
    if docs:
        ...
        logger.info(
            "Indexed %d documents (%d chunks)", len(docs), len(pipeline)
        )
