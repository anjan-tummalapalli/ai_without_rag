...
class TextChunker:
    ...
    def __init__(
        ...
        self.sentence_splitter = sentence_splitter or (
            lambda t: [
                s.strip() for s in self.SENTENCE_SPLIT_RE.split(t) if s.strip()
            ]
        )
    ...
    def chunk_by_sentences_with_meta(
        ...
        for tokens in sent_tokens:
            ...
            if current_tokens and len(current_tokens) + sent_len > max_tokens:
                ...
                overlap_tokens = (
                    current_tokens[-overlap_count:] if overlap_count > 0 else []
                )
    ...
    def chunk_by_sentences(
        ...
        return [
            c.text
            for c in self.chunk_by_sentences_with_meta(
...
class PromptCorrector:
    ...
    def correct_with_meta(
        ...
        if not prompt:
            return []
        if by_sentences:
            return self.chunker.chunk_by_sentences_with_meta(
                prompt, max_tokens=max_tokens
            )
