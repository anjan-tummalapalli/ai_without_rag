...
class Metrics:
        ...
        def __init__(
                ...
                self._enabled = (
                        PROMETHEUS_ENABLED and PromCounter is not None
                ...
                self.latency = _safe_gauge(
                        "ai_gateway_latency_seconds", "Request latency seconds",
                        ["provider"],
                )
...
class ModelQualityMetrics:
        ...
        def avg_embedding_latency(self) -> float:
                ...
                if self.total_embedding_requests <= 0:
                        return 0.0
                return (
                        self.total_embedding_latency_seconds
                        / self.total_embedding_requests
                )
        ...
        def avg_vector_query_latency(self) -> float:
                ...
                if self.total_vector_queries <= 0:
                        return 0.0
                return (
                        self.total_vector_query_latency_seconds
                        / self.total_vector_queries
                )
